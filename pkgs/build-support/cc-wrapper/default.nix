...
, cc ? null, libc ? null, binutils, coreutils ? null, shell ? stdenvNoCC.shell
...
# Ensure binutils matches
assert libc_bin == binutils.libc_bin;
assert libc_dev == binutils.libc_dev;
assert libc_lib == binutils.libc_lib;
assert nativeTools == binutils.nativeTools;
assert nativeLibc == binutils.nativeLibc;
assert nativePrefix == binutils.nativePrefix;
...
stdenv.mkDerivation {
  ...
  inherit cc libc_bin libc_dev libc_lib binutils coreutils_bin;
  ...
  __depsTargetTarget = extraPackages;
