...
let configureFlags = if buildType == "jsffi" then [ ] else if buildType == "webkit" then [ "-f-ghcjs" "-fwebkit" "-f-gtk3" ] else throw "unrecognized buildType";
in cabal.mkDerivation (self: {
  ...
  preConfigure = ''
    configureFlags="${self.stdenv.lib.concatStringsSep " " configureFlags} $configureFlags"
  '';
  meta = {
