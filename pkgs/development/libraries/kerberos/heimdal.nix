...
, sqlite, db, ncurses, openssl, cyrus_sasl, bootstrap_cmds, openbsm
, Security, configd
}:
...
stdenv.mkDerivation rec {
  ...
  configureFlags = [
    ...
    "--with-berkeley-db-include=${db}/include"
    "--without-x"
  ];
  ...
  buildInputs = [
    pkgconfig flex yacc readline openldap sqlite db ncurses cyrus_sasl openssl
  ] ++ stdenv.lib.optional (stdenv.isLinux) libcap_ng
    ++ stdenv.lib.optionals (stdenv.isDarwin) [ bootstrap_cmds openbsm Security configd ];
